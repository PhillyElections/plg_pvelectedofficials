...
class plgContentElectedofficials extends JPlugin
{
    ...
    /**
     * Using officials data, return officials display.
     *
        ...
        foreach ($results as $label => $group) {
            ...
            foreach ($group as $items) {
                foreach ($items as $item) {
                    ...
                    $district = JString::trim($item['congressional_district']) . JString::trim($item['state_senate_district']) . JString::trim($item['state_representative_district']) . JString::trim($item['council_district']);
                    $return .= '	<div class="h-card">';

                    if (in_array($label, array('City Officials', 'State Officials'))) {
                        $return .= '        <div class="p-job-title">' . JText::_($item['office']) . '</div>';
                    }
                    if ($item['website']) {
                        $return .= '		<a class="p-name u-url" href="http://' . $item['website'] . '" target="_blank">' . $fullname . '</a> <sup class="p-note" title="' . JText::_((JString::strtoupper($item['party']) === 'D' ? 'Democratic' : 'Republican')) . '">' . JString::strtoupper($item['party']) . '</sup>';
                    } else {
                        $return .= '		' . $fullname . ' <sup class="p-note" title="' . JText::_((JString::strtoupper($item['party']) === 'D' ? 'Democratic' : 'Republican')) . '">' . JString::strtoupper($item['party']) . '</sup>';
                    }
                    if ($item['leadership_role']) {
                        $return .= '&nbsp;<span class="p-role">(' . JText::_($item['leadership_role']) . ')</span>';
                    }
                    if ($district) {
                        $return .= '        <div class="p-location">' . JText::_('District') . ' ' . $district . '</div>';
                    }
                    $return .= '		<div class="p-adr h-adr">';
                    $return .= '			<div class="p-street-address">' . JText::_($contact_address_1) . '</div>';
                    if ($contact_address_2) {
                        $return .= '			<div class="p-street-address">' . JText::_($contact_address_2) . '</div>';
                    }
                    ...
                    if ($contact_phone) {
                        $num = $contact_phone;
                        $return .= '		<div class="p-tel">' . "(" . JString::substr($num, 0, 3) . ") " . JString::substr($num, 3, 3) . "-" . JString::substr($num, 6) . '</div>';
                    }
                    if ($contact_phone2) {
                        $num = $contact_phone2;
                        $return .= '        <div class="p-tel">' . "(" . JString::substr($num, 0, 3) . ") " . JString::substr($num, 3, 3) . "-" . JString::substr($num, 6) . '</div>';
                    ...
                    if ($contact_fax) {
                        $num = $contact_fax;
                        $return .= '        <div class="p-tel-fax">' . "(" . JString::substr($num, 0, 3) . ") " . JString::substr($num, 3, 3) . "-" . JString::substr($num, 6) . '</div>';
                    }
                    $return .= '	</div>';
                }
            }
    ...
    /**
     * setupContent -- there's a certain amount of juggling of values based on presence of absence of data
     * This is just here to slim getContent a bit.
     *
     * @param    array    $item
     * @param    string   $label
     * @return   array
     */
    public function setupContent($item, $label)
    {
